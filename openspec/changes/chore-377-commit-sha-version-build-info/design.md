...
- Generate a `COMMIT_SHA` `BuildConfig` string during Gradle configuration by first honoring the CI-provided commit SHA and otherwise resolving `git rev-parse HEAD`; use an empty string when neither is available. This supports reproducible CI builds while allowing local checkouts and source archives to build without exposing placeholder text.
...
- [Git unavailable] → Emit an empty identifier and keep the build successful; the UI omits the optional suffix.

...
- **Editor-as-pane only at ≥1200dp; editors stay UiState-owned below it.** At ≥1200dp the editor form state is promoted into a back-stack entry rendered by a supporting-pane scene beside the main content (~412dp pane, content keeps its readable width). Below 1200dp editors remain bottom sheets, with top-origin category editing retaining its direction. The placement function remains the single decision point (`BottomSheet | DockedPane`).
...
- **Apply the M3 breakpoint tiers consistently in PR 2.** Compact (<600dp) keeps bottom navigation and sheet-based sort/editors. Rail-capable compact-landscape and medium/expanded windows (600–1199dp) use the collapsed rail; Settings is pinned to that rail, sort becomes an anchored menu, task/habit editors rise from the bottom, and category editing remains a top sheet. Large/XL (≥1200dp) uses the labeled rail, docked 412dp editor pane, and anchored sort menu.
  - Modal sheets reserve the floating rail's start clearance so the scrim and sheet align to the Tasks/Routines content region rather than covering the whole app shell.
  - Settings remains a Navigation 3 route pushed onto the active section at every width. Compact windows preserve the existing right-to-left pushed transition and back affordance. Rail layouts use the same content transition as Routines/Tasks, keep the rail visible with Settings selected, omit the back affordance, and suppress screen-contextual actions. The collapsed rail reuses the compact circular Settings control and adds its persistent selected treatment; the labeled rail uses a full tab row and omits the redundant in-content Settings title.
  - A docked editor selection is reflected on its source card with a smoothly animated tonal surface and elevation instead of an inserted outline. Activating the selected card again dismisses the pane through the same guarded dismiss path.
...
- **Custom supporting-pane scene over `SupportingPaneSceneStrategy` if the Material artifact fights the floating-pill shell.** Try `androidx.compose.material3.adaptive:adaptive-navigation3` first (it is the documented path per the official adaptive skill); fall back to a small custom `Scene` if its scaffolding imposes pane chrome that conflicts with the custom rail. Decision recorded at implementation time in this doc.
  - Implementation decision: PR 2 uses a small custom Navigation 3 `SceneStrategy`. Editor marker entries carry explicit metadata pairing them with their Routines or Tasks main entry, while the existing screen owns the movable editor content. This keeps the floating-pill shell and existing Hilt entry scope intact, avoids adding a library outside the approved stack, and lets the same editor composition move between modal and docked containers without losing its draft.
...
- **Desktop-style windows on Android (freeform resize)** can cross 1200dp dynamically; pane ↔ bottom-sheet transitions must not lose editor draft state — the form state lives in the ViewModel either way, which is the safeguard.

...
- **Apply the M3 breakpoint tiers consistently in PR 2.** Compact (<600dp) keeps bottom navigation and sheet-based sort/editors. Rail-capable compact-landscape and medium/expanded windows (600–1199dp) use the collapsed rail; Settings is pinned to that rail, sort becomes an anchored menu, task/habit editors rise from the bottom, and category editing remains a top sheet. Large/XL (≥1200dp) uses the labeled rail, docked 412dp editor pane, and anchored sort menu.
  ...
  - At ≥1200dp Settings is a true top-level rail tab that replaces the content pane in place, uses the same tab transition as Routines/Tasks, keeps the rail visible and selected, and suppresses screen-contextual actions. Below 1200dp Settings keeps pushed/back behavior while its rail action remains available whenever the collapsed rail is shown.

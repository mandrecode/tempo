...
### Requirement: Quick-task Done action submits and dismisses

The app SHALL preserve quick-task Done submission while dismissing text entry.

#### Scenario: Done with a non-empty quick-task title

- **WHEN** the user invokes Done after entering a non-blank quick-task title
- **THEN** exactly one quick task is submitted
- **AND** the title input is cleared, loses focus, and the software keyboard is dismissed

#### Scenario: Done with an empty quick-task title

- **WHEN** the user invokes Done with a blank quick-task title
- **THEN** no quick task is submitted
- **AND** the software keyboard is dismissed

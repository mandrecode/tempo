...
- [Risk] `MY_PACKAGE_REPLACED` fires before the updated APK's Hilt graph is warm in some edge cases → Mitigation: `BootAndTimeReceiver` is annotated `@AndroidEntryPoint` but declares no `@Inject` fields of its own — it only touches `WorkManager` directly — so its `onReceive()` has no Hilt-injection ordering dependency; `RescheduleRemindersWorker` itself is a `HiltWorker` resolved lazily by `WorkManager` when it actually runs, not at enqueue time.

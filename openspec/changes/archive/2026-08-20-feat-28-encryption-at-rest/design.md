...
- There is no unencrypted backup import format: export/import (#26) shipped the same day as this change, so there's no real plaintext `.json` backup in the wild to stay compatible with.
...
4. Backup export/import: all exports use the encrypted `.tempo` format from the moment this change ships; there is no unencrypted format to migrate away from, since export/import (#26) landed the same day.

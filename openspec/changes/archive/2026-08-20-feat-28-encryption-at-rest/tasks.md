...
- [x] 5.3 Updated `BackupRepositoryImpl.kt`: `exportToJson()` → `exportEncrypted(passphrase)` wraps the (renamed, now-private) plaintext-JSON builder through `BackupEncryptionService`; `importFromJson` gained an `isEncryptedBackup(content)` peek and a `passphrase: CharArray?` param, decrypting when the content is an encrypted envelope. Export/import (#26) shipped the same day as this change, so there's no legacy plaintext backup format to fall back to — content that isn't a decodable envelope is reported as `ImportOutcome.CorruptFile`; added `ImportOutcome.WrongPassphrase`
- [x] 5.4 Updated the export filename suggestion (`ExportBackupUseCase`) to `backup-<date>-<time>.tempo`
...
- [x] 6.1 Updated `docs/BACKUP_FORMAT.md`: replaced the "exports are unencrypted" callout with an "Encryption" section (envelope shape, KDF params, mandatory passphrase, `.tempo` extension, no unencrypted import format), plus a note in "Evolution rules" that `encryptionVersion` and `schemaVersion` are independent
...
- [x] 7.2 Unit test envelope detection and import in `BackupRepositoryImpl`: encrypted the existing `v1-backup.json` fixture through a real `BackupEncryptionService` and confirmed it imports correctly via `isEncryptedBackup`/`importFromJson`; confirmed non-envelope content (including that same fixture unencrypted) is reported as corrupt rather than imported; added a wrong-passphrase-on-import test and an unsupported-`encryptionVersion` test

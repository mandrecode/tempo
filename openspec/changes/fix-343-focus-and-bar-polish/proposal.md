...
- No change to chain expansion (the chevron) or chain completion. The chain card itself gains no delete affordance; the editor's own delete button works from Focus exactly as the habit editor's already does.
...
- `features/focus/presentation/FocusContent.kt` — chain card tap target, undated footer.

...
class SettingsBackupDelegate
    ...
    constructor(
        ...
        private fun handleExportPassphraseConfirmed(
            ...
            // CharArray content, not reference, must be compared here — the dialog's own
            // confirmEnabled check already guards this in practice, but this stays correct
            // defensively regardless.
            if (event.passphrase.isEmpty() || !event.passphrase.contentEquals(event.confirmation)) {
                event.passphrase.fill(' ')
            ...
            host.scope.launch {
                ...
                try {
                    val export = exportBackup(event.passphrase)
                    ...
                    // Wipe both CharArrays as soon as the export use case is done with them,
                    // same as the import passphrase's clearImportPassphrase().
                    event.passphrase.fill(' ')
                    event.confirmation.fill(' ')
        ...
        private fun handleImportFilePicked(
            ...
            importReadJob =
                host.scope.launch {
                    ...
                    // Every valid export is an encrypted envelope — there's no legacy
                    // unencrypted backup format to fall back to — so anything else is
                    // reported as corrupt right away instead of prompting for a passphrase.
                    val dialog =
                        if (backupRepository.isEncryptedBackup(content)) {
                            ...
                            SettingsContract.BackupDialog.ImportFailed(
                                SettingsContract.ImportError.CorruptFile,
                            )
                        }
